...
exports.toBSON = function (string) {
  ...
  string = string.replace(/DBRef\(([^)]+),([^)]+),([^)]+)\)/g, '{ "$ref": $1, "$id": $2, "$db": $3 }');
  string = string.replace(/DBRef\(([^)]+),([^)]+)\)/g, '{ "$ref": $1, "$id": $2, "$db": ""}');
  string = string.replace(/DBRef\(([^)]+)\)/g, '{ "$ref": $1, "$id": "", "$db": "" }');
...
exports.toString = function (doc) {
  ...
  var string = json.stringify(doc, null, '    ');
  return string;
};
...
exports.toJsonString = function (doc) {
  ...
  sJson = sJson.replace(/DBRef\(([^)]+),([^)]+),([^)]+)\)/g, '{ "$ref": $1, "$id": $2, "$db": $3 }');
  sJson = sJson.replace(/DBRef\(([^)]+),([^)]+)\)/g, '{ "$ref": $1, "$id": $2, "$db": ""}');
  sJson = sJson.replace(/DBRef\(([^)]+)\)/g, '{ "$ref": $1, "$id": "", "$db": "" }');
